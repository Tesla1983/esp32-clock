...
const unsigned long updateInterval = 1000; // 1秒
...
void updateTimeDisplay(float temperature) {
  ...
  tft.drawString("日期:", 0, 28, 4);
  tft.drawString(dateStr, 50, 28, 4);
  tft.drawString("温度: " + String(temperature, 1) + "℃", 0, 55, 4);
  
  tft.drawString(dayOfWeekStr, 90, 2, 4);
  tft.drawString("湿度: " + String(humidity, 1) + "%", 0, 80, 4);
}
...
void loop() {
  ...
  if (currentMillis - lastUpdate >= updateInterval) {
    ...
    lastUpdate = currentMillis; // 重置计时器
  }
}
